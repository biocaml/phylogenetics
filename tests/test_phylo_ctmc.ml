...
let test_pruning ?(tree_size = 5) ?(seq_size = 10) () =
  ...
  let ctmc_result =
    ...
    let transition_matrix l = (M.transition_probability_matrix () l :> Linear_algebra.Owl.mat) in
    let root_frequencies = (M.stationary_distribution () :> Linear_algebra.Owl.vec) in
