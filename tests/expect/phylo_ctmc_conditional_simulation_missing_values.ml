...
let small_tree =
  let open Tree in
  binary_node ()
    (branch 0.4 (binary_node ()
                   (branch 0.8 (leaf "A1"))
                   (branch 1.2 (leaf "A2"))))
    (branch 0.1 (binary_node ()
                   (branch 0.6 (binary_node ()
                                  (branch 1.2 (leaf "B1"))
                                  (branch 0.3 (leaf "B2"))))
                   (branch 0.3 (leaf "C"))))
...
let reference_simulation, profile, rate_matrix =
  ...
  in
  let censored_sim =
    Tree.map sim ~node:Fun.id ~branch:Fun.id ~leaf:(fun (label, nuc) ->
        label,
        if String.equal label "B2" then None else Some nuc)
  in
  censored_sim, profile, rate_matrix
...
let conditional_simulations =
  ...
  in
  let cl =
    Phylo_ctmc.Missing_values.conditional_likelihoods
      reference_simulation
      ~nstates:Nucleotide.card
      ~leaf_state:(fun (_, nuc) -> Option.map ~f:Nucleotide.to_int nuc)
  ...
  Array.init _B_ ~f:(fun _ ->
      Phylo_ctmc.Missing_values.conditional_simulation rng cl ~root_frequencies
      |> map_tree
    )
...
let rejection_sampling_simulations =
  ...
  let rec loop () =
    ...
    let compatible (l1, nuc1) (l2, maybe_nuc2) =
      String.equal l1 l2
      && Option.value_map ~default:true maybe_nuc2 ~f:(Nucleotide.equal nuc1)
    in
    if List.for_all2_exn ~f:compatible leaves reference_leaves then map_tree sim
    else loop ()
  in
