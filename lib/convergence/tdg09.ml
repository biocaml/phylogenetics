...
module Model1 = struct
  ...
  let inner_maximum_log_likelihood ?debug ~exchangeability_matrix ~stationary_distribution site =
    let f vec =
  ...
  let maximum_log_likelihood ?debug ~exchangeability_matrix ~stationary_distribution site =
    let ll, vec = inner_maximum_log_likelihood ?debug ~exchangeability_matrix ~stationary_distribution site in
    ll, decode_vec vec
...
module Model2 = struct
  ...
  let log_likelihood ~exchangeability_matrix ~param:{ stationary_distribution ; scale } site =
  ...
  let inner_maximum_log_likelihood ?debug ?mode ~exchangeability_matrix site =
    ...
    let f p =
      ...
      -. log_likelihood ~exchangeability_matrix ~param site
    in
  ...
  let maximum_log_likelihood ?debug ?mode ~exchangeability_matrix site =
    let ll, schema, vec = inner_maximum_log_likelihood ?debug ?mode ~exchangeability_matrix site in
  ...
  let lrt ?mode (wag : Wag.t) sample =
    let reduced_log_likelihood, p1 = Model1.maximum_log_likelihood ~exchangeability_matrix:wag.rate_matrix ~stationary_distribution:wag.freqs sample in
    let full_log_likelihood, schema, p2 = inner_maximum_log_likelihood ?mode ~exchangeability_matrix:wag.rate_matrix sample in
...
module Model3 = struct
  ...
  let evolution_model_param exchangeability_matrix param cond =
    ...
    in
    match cond with
    | 0 -> f param.stationary_distribution0
    | 1 -> f param.stationary_distribution1
    | _ -> assert false

  let log_likelihood ~exchangeability_matrix ~param site =
    let f cond =
      evolution_model_param exchangeability_matrix param cond
      |> Evolution_model.transition_probability_matrix
    in
    let transition_matrix =
      let f0 = f 0 in (* pre-computation *)
      let f1 = f 1 in
      fun (bl, cond) ->
        match cond with
        | 0 -> (f0 bl :> mat)
    ...
    CTMC.pruning site ~transition_matrix ~leaf_state:Fn.id ~root_frequencies
  ...
  let lrt ?mode (wag : Wag.t) tree sample =
    let exchangeability_matrix = wag.rate_matrix in
    let reduced_log_likelihood, schema2, p2 = Model2.inner_maximum_log_likelihood ?mode ~exchangeability_matrix sample in
    let full_log_likelihood, schema3, p3 = inner_maximum_log_likelihood ?mode ~model2_opt:p2 ~exchangeability_matrix tree sample in
  ...
  let simulate_site exchangeability_matrix tree scale stationary_distribution0 stationary_distribution1 =
    let param =
      { scale ; stationary_distribution0 ; stationary_distribution1 }
      |> evolution_model_param exchangeability_matrix
    in
  ...
  let f _ =
    let sample =
      Model1.simulate_site
        ~exchangeability_matrix:wag.rate_matrix
        ~stationary_distribution:wag.freqs
        ~param:true_scale tree in
    let p1, p2, lrt = Model2.lrt ?mode wag sample in
    sample, p1, p2, lrt
  in
  Array.init 1_000 ~f
  ...
  let f _ =
    ...
    let sample =
      Model1.simulate_site
        ...
        ~param:true_scale tree in
    let p2, p3, lrt = Model3.lrt ?mode wag tree sample in
    sample, p2, p3, lrt
  in
