...
struct
  ...
  let eMt_mat e t =
    ...
    mult (mult diag_p (scal_mat_mul diag t)) diag_p_inv
...
module Make (M:TRANSITION_MATRIX) = struct
  module Diag = struct
    ...
    let diag_mats p = LATools.diagonalize (transition_mat p)
  end
...
module JC69 =
  Make_exp (struct
    ...
    let diag_mats () =
      init_mat 4 (fun i j ->
          if i = j then match i with 1 -> 0.0 | _ -> -4./.3.
          else 0.0),
      init_mat 4 (fun i j ->
          if j=1 then 1.0
